...
export function bionify() {
  ...
  chrome.storage.sync.get(["algorithm"], (data) => {
    ...
    function bionifyifyWord(word) {
      // Determine the number of characters to make red
      var numRed = Math.min(3, word.length);
    
      // Create the red part of the word
      var redPart = '<span style="color: red;">' + word.slice(0, numRed) + '</span>';
    
      // Create the rest part of the word
      var restPart = word.slice(numRed);
    
      // Combine the red part and the rest part
      return redPart + restPart;
    }
